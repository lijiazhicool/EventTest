...
/**
 * 可以水平滑动的ViewGroup 总有有以下几个知识点: 1. VelocityTracker:用来检测速度,如果水平速度达到一定阈值则切换页面 2.
 * Scroller:弹性滑动,水平滑动到一半的时候,放开以后View滑动到标准的某个页面的位置 3.
 * onInterceptTouchEvent:拦截机制,也是解决滑动冲突,当水平位移大于垂直位移的时候就有当前组件处理触摸事件而不是子View 4.
 * 自定义ViewGroup：需要自己实现onMeasure（尤其要处理wrap_content的情况），自己去实现onLayout（需要去调用每一个子View的layout函数去布局子View） Created by leoray on
 * 16/1/15.
 */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: // 释放手指以后开始自动滑动到目标位置
                ...
                currentIndex =
                    currentIndex < 0 ? 0 : currentIndex > getChildCount() - 1 ? getChildCount() - 1 : currentIndex;
